...
function filterByTimeRange(executions, range) {
  if (
    typeof filterByTimeRange === "undefined" &&
    typeof window.filterByTimeRange === "function"
  ) {
...
function updateStats() {
  ...
  document.querySelectorAll(".stat-subtitle").forEach((el, index) => {
    ...
    if (index === 2)
      el.textContent = stats.failed > 0 ? "Needs attention" : "All good";
  });
}
...
function updateFilterCounts() {
  ...
  const failedCount = filteredExecutions.filter(
    (e) => e.status === "failed",
  ).length;
...
function analyzeFailure(executionId) {
  ...
  if (execution && execution.error) {
    askConsole(
      `Why did execution ${executionId} fail? Error: ${execution.error}`,
    );
  } else {
...
function askConsole(query) {
  ...
  const logsIsDev =
    logsConsoleHostname === "localhost" || logsConsoleHostname === "127.0.0.1";
  const logsConsoleUrl = logsIsDev
    ? "http://localhost:3000/console"
    : `${window.location.protocol}//${logsConsoleHostname}:3000/console`;

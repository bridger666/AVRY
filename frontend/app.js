...
// Dashboard URL Configuration - Environment-aware with configuration fallbacks
...
if (!window.DASHBOARD_URL) {
  ...
  const configuredUrl = htmlElement?.getAttribute("data-dashboard-url");
  const storedUrl = localStorage.getItem("aivory_dashboard_url");

  if (configuredUrl) {
    // Explicit data attribute takes precedence
    window.DASHBOARD_URL = configuredUrl;
  } else if (storedUrl) {
...
const FREE_DIAGNOSTIC_QUESTIONS = [
  ...
  },
  {
    ...
    options: ["No alignment", "Some interest", "Supportive", "Championing AI"],
  },
  {
    id: "budget_ownership",
...
function renderFreeQuestion() {
  ...
  // Update progress
  const progress =
    ((currentQuestionIndex + 1) / FREE_DIAGNOSTIC_QUESTIONS.length) * 100;
  ...
  if (nextButton)
    nextButton.disabled = freeDiagnosticAnswers[question.id] === undefined;
  ...
  if (currentQuestionIndex === FREE_DIAGNOSTIC_QUESTIONS.length - 1) {
    ...
    if (submitButton)
      submitButton.style.display =
        freeDiagnosticAnswers[question.id] !== undefined ? "block" : "none";
  } else {
...
async function submitFreeDiagnostic() {
  ...
  if (
    Object.keys(freeDiagnosticAnswers).length !==
    FREE_DIAGNOSTIC_QUESTIONS.length
  ) {
  ...
  try {
    ...
    const formattedAnswers = Object.entries(freeDiagnosticAnswers).map(
    ...
    const userEmail = document
      .getElementById("free-result-email")
      ?.value?.trim();
    ...
    if (
      ...
      typeof IDChainManager !== "undefined"
    ) {
      IDChainManager.storeDiagnosticData(freeDiagnosticResult.diagnostic_id, {
    ...
    alert(
      "Failed to run diagnostic. Please ensure the backend is running on port 8081 and try again.",
    );
    if (loadingEl) loadingEl.style.display = "none";
    if (questionsEl) questionsEl.style.display = "block";
  }
...
function displayFreeDiagnosticResults(result) {
  ...
  const categoryExplanation = document.getElementById(
    "free-category-explanation",
  );
  ...
  if (categoryExplanation)
    categoryExplanation.textContent = result.category_explanation;
...
async function saveAndEmailResults() {
  ...
  if (!resultJson) {
    if (statusMessage) {
      statusMessage.textContent =
        "Error: No results found. Please retake the diagnostic.";
      statusMessage.style.color = "#E74C3C";
    }
    return;
  }
  ...
  try {
    ...
    if (statusMessage) {
      statusMessage.textContent =
        "✓ Results saved! Check your email for the full report.";
...
function accessDashboard(tier = "free") {
  ...
  window.location.href = `${DASHBOARD_URL}/dashboard?tier=${tier.toLowerCase()}`;
}
...
function displayUpgradeOptions() {
  ...
  if (score <= 40) {
    ...
    blueprintContext =
      "Build your foundation first with AI Snapshot, then scale with the full Blueprint.";
  } else if (score <= 60) {
    // AI Emerging
    snapshotContext = "You're one step away from clarity.";
    blueprintContext =
      "Ready to accelerate? AI System Blueprint will help you architect a scalable AI system across your organization.";
  } else if (score <= 80) {
    // AI Operational
    snapshotContext = "You're one step away from clarity.";
    blueprintContext =
      "You have strong foundations. AI System Blueprint will help you architect a scalable AI system across your organization.";
  } else {
    // AI Transformational
    snapshotContext = "You're one step away from clarity.";
    blueprintContext =
      "You're ahead of the curve. AI System Blueprint will help you optimize and scale what's already working.";
  }
...
function renderSnapshotQuestion() {
  ...
  if (typeof SNAPSHOT_DIAGNOSTIC_QUESTIONS === "undefined") {
    console.error(
      "SNAPSHOT_DIAGNOSTIC_QUESTIONS not loaded. Please include diagnostic-questions-snapshot.js",
    );
  ...
  // Update progress
  const progress =
    ((currentQuestionIndex + 1) / SNAPSHOT_DIAGNOSTIC_QUESTIONS.length) * 100;
  ...
  if (prevButton) prevButton.disabled = currentQuestionIndex === 0;
  if (nextButton)
    nextButton.disabled = snapshotDiagnosticAnswers[question.id] === undefined;
  ...
  if (currentQuestionIndex === SNAPSHOT_DIAGNOSTIC_QUESTIONS.length - 1) {
    ...
    if (submitButton)
      submitButton.style.display =
        snapshotDiagnosticAnswers[question.id] !== undefined ? "block" : "none";
  } else {
    if (nextButton) nextButton.style.display = "block";
    if (submitButton) submitButton.style.display = "none";
  }
...
async function submitSnapshotDiagnostic() {
  ...
  // Validate all questions answered
  if (
    Object.keys(snapshotDiagnosticAnswers).length !==
    SNAPSHOT_DIAGNOSTIC_QUESTIONS.length
  ) {
  ...
  try {
    // Format answers for API
    const formattedAnswers = Object.entries(snapshotDiagnosticAnswers).map(
      ([question_id, selected_option]) => ({
        question_id,
        selected_option,
      }),
    ...
    if (userContext.company_name)
      requestBody.company_name = userContext.company_name;
    ...
    // ✅ STORE SNAPSHOT ID
    if (
      snapshotDiagnosticResult.snapshot_id &&
      typeof IDChainManager !== "undefined"
    ) {
...
function startBlueprint() {
  ...
  if (!snapshotDiagnosticCompleted || !snapshotDiagnosticResult) {
    ...
    if (!snapshotId) {
      alert(
        "Please complete the AI Snapshot ($15) diagnostic first! Blueprint requires Snapshot data.",
      );
      startSnapshot();
      return;
    }
...
async function runBlueprint() {
  ...
  try {
    ...
    const isSuperAdmin =
      typeof AuthManager !== "undefined" && AuthManager.isSuperAdmin();
    const user =
    ...
    if (isSuperAdmin && !snapshotId) {
      console.log(
        "🔧 Super admin: No snapshot_id found, injecting mock snapshot_id",
      );
    ...
    alert(
      `Failed to generate AI System Blueprint: ${error.message}\n\nPlease try again or contact support.`,
    );
    showSection("snapshot-diagnostic");
  }
}
...
async function displayBlueprintResults(result) {
  try {
    ...
    alert(
      "Blueprint generated but failed to load data. Please try refreshing the dashboard.",
    );
    window.location.href = `${DASHBOARD_URL}/dashboard?mode=blueprint`;
  }
}
...
function addCredits(amount) {
  ...
  alert(
    `Adding ${amount} Intelligence Credits to your account. This feature will be available soon.`,
  );
}
...
function selectPlan(planName) {
  ...
  alert(
    `You selected the ${planName} plan. This feature will be available soon.`,
  );
}
 ...
 * Routes by role: admin/employee → admin.aivory.id, regular user → app.aivory.id
 */
